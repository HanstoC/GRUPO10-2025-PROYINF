...
onMount(async () => {
  ...
  try {
    ...
    const resAll = await fetch(`http://localhost:8000/preguntas?asignatura=${ensayo.id_asignatura}`, {
      credentials: 'include'
    });
...
{:else}
  ...
  <h2 class="text-lg font-semibold mt-4">Preguntas actuales</h2>
  <ul class="mt-2">
  ...
  <h2 class="text-lg font-semibold mt-8">Agregar nuevas preguntas</h2>
  <ul class="mt-2">
  ...
  <button on:click={guardar} class="mt-6 bg-blue-600 text-white px-4 py-2 rounded">
    Guardar cambios
  </button>
{/if}